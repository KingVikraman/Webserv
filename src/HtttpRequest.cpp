...
void HttpRequest::_handle_headers()
{
    ...
    if (_request_headers.count("content-type") && _request_headers["content-type"].find("multipart/form-data") != std::string::npos)
    {
        ...
        if (pos != std::string::npos)
            this->_boundary = _request_headers["content-type"].substr(pos + 9, _request_headers["content-type"].size()); // ignore "boundary=" and get the rest as boundary string
    ...
    for (size_t i = 0; i < size; ++i)
    {
        ...
        switch (_state)
        {
            ...
            case Request_Line_Method: // check the rest of the string to make sure correct
            {
                ...
                if (_method_index == static_cast<int>(_method_str[_method].length()))
            ...
            {
                if (character == '\r') // end of headers, now expect \n to confirm end of headers
                    _state = Fields_End;
                else if (isToken(character)) // if valid move to field name state
                    _state = Field_Name;
                else
                {
                    _error_code = 400;
            ...
            case Fields_End:
            {
                if (character == '\n')
                {
                    ...
                    {
                        if (_chunked_flag == true) // if chunked transfer encoding, move to chunked length parsing state
            ...
            {
                if (isxdigit(character) == 0) // check valid hex digit
                {
                    _error_code = 400;
            ...
            case Chunked_Length:
            {
                if (isxdigit(character) != 0)
                {
                    ...
                    s.clear();
                    s << character;
                    s >> std::hex >> temp_len; // basically num * 16 + 10 and repeat per hex digit
                    _chunk_length *= 16;
            ...
            case Chunked_Length_LF:
            {
                if (character == '\n')
                {
                    if (_chunk_length == 0)
                        ...
                        _state = Chunked_Data; // more chunks to read, move to chunk data state
                }
                else
                {
                    _error_code = 400;
